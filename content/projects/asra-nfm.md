---
slug: asra-nfm
title: ASRA-NFM
description: "Neuro-symbolic adaptive reasoning architecture that learns world dynamics through intervention, represents discovered mechanisms explicitly, and plans actions under uncertainty — object-centric states, transition world models, causal hypotheses, and reproducible experiment loops for ARC-AGI-style environments."
role: "Creator — ASRA-NFM architecture (Phases 1–5), symbolic+neural hybrid world models, Tabby/Grubby/Sam metacognitive loop, ARC-AGI-3 adapters, evaluation harness, and theory poster/presentation pack"
status: Active
featured: true
githubUrl: ""
websiteUrl: "/projects/asra-nfm"
demoVideoUrl: ""
caseStudyUrl: ""
relatedLinks: '[{"label":"Paper concepts","url":"/asra-nfm/paper/paper1.md"},{"label":"Poster (PDF)","url":"/asra-nfm/posters/asra-nfm-poster-16x9.pdf"},{"label":"Slides (PDF)","url":"/asra-nfm/presentation/asra-nfm-slides.pdf"},{"label":"ASRA","url":"/asra"}]'
filterTags: '["AI / ML","Scientific AI","Open Source"]'
techStack: '["Python","Neuro-symbolic reasoning","Object-centric states","Transition world models","Causal interventions","Planning under uncertainty","ARC-AGI","PyTorch"]'
---

**ASRA-NFM** is an adaptive neuro-symbolic reasoning architecture for Nature Foundation Models: learn world dynamics through intervention, represent discovered mechanisms explicitly, and plan actions under uncertainty.

It combines object-centric perception, before–action–after transition logs, competing symbolic hypotheses, neural + symbolic world models, and active experiment selection (Tabby / Grubby / Sam). Implementation through Phase 5 includes hardened synthetic envs and ARC-AGI-3 mock + Toolkit adapters.

**Project page:** [/projects/asra-nfm](/projects/asra-nfm)
