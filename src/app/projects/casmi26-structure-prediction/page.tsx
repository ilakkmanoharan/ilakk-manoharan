...
import { Casmi26ScoreChart } from "@/components/casmi26-score-chart";
import {
  formatCasmiScore,
  formatScoreDelta,
  listCasmiDayDates,
...
export const metadata: Metadata = {
  ...
  description:
    "Daily research log for Enveda CASMI 2026 molecule ID from mass spectra: Kaggle scores, hypotheses, concepts, and ASRA-aligned analysis.",
};

export default function Casmi26ProjectPage() {
  const config = loadCasmiConfig();
  const history = loadCasmiScoreHistory();
  const dates = listCasmiDayDates().reverse();
  const latestDate = dates[0];
  const latestDay = latestDate ? loadCasmiDay(latestDate) : null;
  ...
  return (
    <>
      <ViewTracker
        path="/projects/casmi26-structure-prediction"
      ...
      <div className="mx-auto max-w-6xl px-4 py-12 md:px-6 md:py-16">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Badge variant="secondary">Mass spectrometry</Badge>
        </div>
        ...
        <p className="mt-3 max-w-3xl text-muted-foreground">
          ...
          <a
            href={config.competitionUrl || CASMI_KAGGLE}
            target="_blank"
            rel="noreferrer"
        ...
        <div className="mt-8 flex flex-wrap gap-3 text-sm">
          <a
            href={config.githubUrl || CASMI_GITHUB}
            target="_blank"
            rel="noreferrer"
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardTitle className="font-heading text-3xl">
                  {formatCasmiScore(history.bestPublicScore)}
                </CardTitle>
              </CardHeader>
            ...
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardTitle className="font-heading text-3xl">
                  {formatCasmiScore(history.latestPublicScore)}
                </CardTitle>
              </CardHeader>
              <CardContent className="text-xs text-muted-foreground">
            ...
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardTitle className="text-base leading-snug">
                  {latestDate ? (
                    <Link
                      href={`/projects/casmi26-structure-prediction/daily/${latestDate}`}
                      className="text-primary hover:underline"
                    >
                      {latestDate}
                    </Link>
        ...
        <section className="mt-12" aria-labelledby="score-heading">
          ...
          <Card className="mt-6">
            <CardContent className="pt-6">
              <Casmi26ScoreChart history={history} />
            </CardContent>
          </Card>
        </section>
        ...
        <section className="mt-12" aria-labelledby="calendar-heading">
          ...
          <Card className="mt-6">
            <CardContent className="pt-6">
              <Casmi26DayCalendar dates={[...dates].reverse()} />
            </CardContent>
          </Card>
        </section>
        ...
        <section className="mt-12" aria-labelledby="recent-heading">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            {dates.slice(0, 6).map((d) => {
              const day = loadCasmiDay(d);
              return (
                <Card key={d}>
                  <CardHeader>
                    <CardTitle className="font-heading text-lg">
                      <Link
                        href={`/projects/casmi26-structure-prediction/daily/${d}`}
                    ...
                    <CardDescription>
                      Best{" "}
                      {formatCasmiScore(day?.scoreSummary?.bestScore ?? null)} ·{" "}
