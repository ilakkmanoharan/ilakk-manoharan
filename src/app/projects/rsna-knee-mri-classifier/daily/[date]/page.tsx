...
export async function generateStaticParams() {
  return listRsnaDayDates().map((date) => ({ date }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { date } = await params;
  return {
    title: `RSNA Knee — ${date}`,
    description: `Daily research page for RSNA Knee MRI Classifier on ${date}.`,
  };
}
...
export default async function RsnaKneeDailyPage({ params }: Props) {
  const { date } = await params;
  const day = loadRsnaDay(date);
  if (!day) notFound();

  const dates = listRsnaDayDates();
  const s = day.scoreSummary;

  return (
    <>
      <ViewTracker
        path={`/projects/rsna-knee-mri-classifier/daily/${date}`}
      ...
      <div className="mx-auto max-w-6xl px-4 py-12 md:px-6 md:py-16">
        <p className="text-sm text-muted-foreground">
          <Link
            href="/projects/rsna-knee-mri-classifier"
            className="text-primary hover:underline"
          >
            ← RSNA Knee MRI Classifier
          </Link>
        </p>
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-3">
                ...
                <dl className="grid gap-3 sm:grid-cols-2 text-sm">
                  <div>
                    ...
                    <dd className="font-medium">
                      {formatRsnaScore(s.firstScored)} →{" "}
                      {formatRsnaScore(s.lastScored)} (
                  ...
                  <div>
                    ...
                    <dd className="font-medium">
                      {formatRsnaScore(s.bestScore)} /{" "}
                      {formatRsnaScore(s.worstScore)}
                    </dd>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <dd className="font-medium">
                      {formatRsnaScore(s.meanScore)} /{" "}
                      {formatRsnaScore(s.medianScore)}
                    </dd>
                  </div>
                  <div>
          ...
          <Card>
            ...
            <CardContent>
              <RsnaKneeDayCalendar dates={dates} selected={date} />
            </CardContent>
          </Card>
        </div>
                  ...
                  day.submissions.map((sub) => (
                    <tr key={sub.ref} className="border-b last:border-0">
                      ...
                      <td className="px-3 py-2">
                        {formatRsnaScore(sub.publicScore)}
                      </td>
